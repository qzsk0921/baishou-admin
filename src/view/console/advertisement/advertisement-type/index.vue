...
<script>
import Tab from '@/components/Tab/advertisement/advertisement-type'
import Console from '@/components/console/console'
import Search from '@/components/Search'
import {
  getAdvertisementTypeList,
  removeAdvertisementType
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const tip =
        row.length === 1
          ? '确认删除（' + row[0].typeName + '）吗？'
          : '确认批量删除吗？'
      this.$confirm(tip)
        .then(_ => {
          const id = row.map(item => item.id)
          this.removeAdvertisementType(id)
    ...
    removeAdvertisementType(id) {
      removeAdvertisementType({ id }, '.main-container').then(res => {
        this.getAdvertisementTypeList()
      })
    }
  },
