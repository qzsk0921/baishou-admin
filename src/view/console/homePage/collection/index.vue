...
<script>
import Tab from '@/components/Tab/homePage/collection'
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      if (val.length === 1) {
        ...
        this.getOneCollection(val[0].id)
    ...
    removeCollection(id) {
      removeCollection({ id }, '.main-container').then(res => {
        this.getCollectionList()
      })
    },
    getOneCollection(id) {
      getOneCollection(id, '.tab-container').then(res => {
