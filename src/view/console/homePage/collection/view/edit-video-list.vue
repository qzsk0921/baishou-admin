<template>
  <div class="edit-video-container edit-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb-container">
      <el-breadcrumb-item :to="{ path: '/console/collection' }">合辑</el-breadcrumb-item>
      <el-breadcrumb-item>编辑视频列表</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="tit">编辑视频列表</div>
    <div class="content">
...
import { addVideoToCollection, getVideoListCollection } from '@/api/home-page'
...
export default {
  ...
  data() {
    return {
      ...
      currentVideoListCollection: null
    }
  },
  watch: {
    currentPage(nv) {
  ...
  methods: {
    ...
    handleSelect(val, row) {
      const result = this.currentVideoListCollection.some((item, index) => {
        if (item.id === row.id) {
          // 已经有的，勾选则取消
          this.currentVideoListCollection.splice(index, 1)
        }
        return item.id === row.id
      })
      // 没有的话，勾选则新增
      if (!result) this.currentVideoListCollection.push({ id: row.id })
    },
    ...
    handleSave() {
      const sourceId = this.currentVideoListCollection.map(_ => _.id)
    ...
    addVideoToCollection(data) {
      addVideoToCollection(data, '.class-container').then(res => {
        this.toConsoleCollectioin()
      })
    },
    // 路由跳转
    ...
    toggleRowSelection(data) {
      if (this.currentVideoListCollection.length > 0) {
        this.currentVideoListCollection.forEach(item => {
          data.some((it, ind) => {
    ...
    getVideoListCollection(id) {
      getVideoListCollection(id).then(res => {
        this.currentVideoListCollection = JSON.parse(JSON.stringify(res.object))
      })
    },
    mounted() {
      if (!this.params.videoListCollection) {
        // 直接地址栏输入网址
        this.getVideoListCollection(this.query.id, 'tbody')
      } else {
        // 点击编辑按钮进入
        this.currentVideoListCollection = JSON.parse(
          JSON.stringify(this.params.videoListCollection)
        )
      }
