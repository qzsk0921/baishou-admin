...
<script>
import { addNaventry, updateNaventry, getNavbarAll } from '@/api/home-page'
  ...
  methods: {
    ...
    handleCreate() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) this.addNaventry()
      })
    },
    // 修改密码之外其它信息
    handleEdit() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) this.updateNaventry()
      })
    },
    addNaventry() {
      addNaventry(this.myRow).then(res => {
        this.toConsoleNaventry()
      })
    },
    updateNaventry() {
      updateNaventry(this.myRow).then(res => {
        this.toConsoleNaventry()
      })
    },
    // 路由跳转
