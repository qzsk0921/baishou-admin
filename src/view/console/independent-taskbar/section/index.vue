...
<script>
import Tab from '@/components/Tab/explore/section'
import Console from '@/components/console/console'
import Search from '@/components/Search'
import { getExploreSectionList, getOneExploreSection, removeExploreSection } from '@/api/explore'
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      if (val.length === 1) {
        ...
        this.getOneExploreSection(val[0].id)
    ...
    // 当select选择删除时触发
    handleDelete(row) {
      this.$confirm('确认删除（' + row.sectionName + '）吗')
        .then(_ => {
          this.removeExploreSection()
    ...
    removeExploreSection() {
      removeExploreSection({ id: this.currentRow.id }, '.main-container').then(res => {
        this.getExploreSectionList()
      })
    },
    getOneExploreSection(id) {
      getOneExploreSection(id).then(res => {
