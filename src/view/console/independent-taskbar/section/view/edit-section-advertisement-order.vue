<template>
  <div class="edit-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb-container">
      ...
      <el-breadcrumb-item>内容版块广告排序</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="tit">内容版块：{{params.row.sectionName}}</div>
    <div class="table-container">
      <el-table class="tableData" :data="tableData">
        <el-table-column width="240" prop="advertName" label="广告名称"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit() {
      ...
      setSectionAdvertisementNum(data, '.class-container').then(res => {
        this.toConsoleExploreSection()
      })
    },
    // 路由跳转
    toConsoleExploreSection() {
      this.$router.push({
        name: 'ConsoleExploreSection'
      })
    },
    getSectionAdvertList(id) {
      getSectionAdvertList(id, '.class-container').then(res => {
    ...
    mounted() {
      if (!this.params.advertisementListOfSection) {
        // 直接地址栏输入网址
        this.getSectionAdvertList(this.query.id)
      } else {
        // 点击编辑按钮进入
