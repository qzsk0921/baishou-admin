<template>
  <div class="edit-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb-container">
      <el-breadcrumb-item :to="{ path: '/console/navbar' }">区域</el-breadcrumb-item>
      <el-breadcrumb-item>{{params.status==="create"?"创建":"修改"}}</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="tit">{{params.status==="edit"?"修改地区":"创建地区"}}</div>
    <el-form
      class="ruleForm"
      ref="dataForm"
      :rules="rules"
...
<script>
import { addGegion, updateGegion } from '@/api/video'
...
export default {
  ...
  methods: {
    ...
    handleCancel() {
      this.toConsoleRegion()
    },
    // 创建
    handleCreate() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) this.addGegion()
      })
    },
    // 修改密码之外其它信息
    handleEdit() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) this.updateGegion()
      })
    },
    addGegion() {
      addGegion(this.params.row, 'body').then(res => {
        this.toConsoleRegion()
      })
    },
    updateGegion() {
      ...
      updateGegion(this.params.row, 'body').then(res => {
        this.toConsoleRegion()
      })
    },
    // 路由跳转
